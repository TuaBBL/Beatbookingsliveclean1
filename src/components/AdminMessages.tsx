...
import { MessageSquare, Send, Users, ArrowLeft, Clock } from 'lucide-react';
...
export default function AdminMessages() {
  ...
  useEffect(() => {
    if (loading) return;
    loadConversations();
    const interval = setInterval(loadConversations, 7000);
                  ...
                  {conversations.map((conv) => (
                    <button
                      ...
                      className={`w-full p-4 text-left transition hover:bg-neutral-900 ${
                        selectedUserId === conv.user_id ? 'bg-neutral-900' : ''
                      }`}
                    >
                  ...
                  <div className="max-w-4xl mx-auto">
                    ...
                    <div className="flex items-center gap-3 text-sm text-gray-400">
                      ...
                      <span className={`px-2 py-0.5 rounded text-xs ${
                        selectedConversation.role === 'artist'
                          ? 'bg-blue-500/10 text-blue-400'
                          : 'bg-green-500/10 text-green-400'
                      }`}>
                        {selectedConversation.role}
                      </span>
                      {selectedUserProfile?.last_active_at && (
                        <>
                          <span>•</span>
                          <div className="flex items-center gap-1">
                            <Clock className="w-3 h-3" />
                            <span>Active {formatRelativeTime(selectedUserProfile.last_active_at)}</span>
                          </div>
                        </>
                      )}
                    </div>
